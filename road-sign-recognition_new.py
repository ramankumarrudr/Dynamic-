            ...
            if dominant_color[2] > 100:
                ...
                if zone_1_color[2] < 60:
                    if sum(zone_0_color) > sum(zone_2_color):
                        print("LEFT")
                        #obj = gTTS(text="LEFT",lang = 'en' , slow=False)
                        #obj.save("audio.mp3")
                        #playsound('audio.mp3')
                    else:
                        print("RIGHT")
                        #obj = gTTS(text="RIGHT",lang = 'en' , slow=False)
                        #obj.save("audio1.mp3")
                        #playsound('audio1.mp3')
                else:
                    if sum(zone_1_color) > sum(zone_0_color) and sum(zone_1_color) > sum(zone_2_color):
                        print("FORWARD")
                        #obj = gTTS(text="Forward",lang = 'en' , slow=False)
                        #obj.save("audio2.mp3")
                        #playsound('audio2.mp3')
                    elif sum(zone_0_color) > sum(zone_2_color):
                        print("FORWARD AND LEFT")
                        #obj = gTTS(text="Forward and Left",lang = 'en' , slow=False)
                        #obj.save("audio3.mp3")
                        #playsound('audio3.mp3')
                    else:
                        print("FORWARd and right")
                        #obj = gTTS(text="Forward and right",lang = 'en' , slow=False)
                        #obj.save("audio4.mp3")
